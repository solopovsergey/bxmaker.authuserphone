...
// запрос проверки подтверждения,  проверка звонка от опользвоателя
...
try {
    ...
    $checkUserCallResult = $oManagerAuthUserPhone->service()->checkUserCall($phone, $code);
    if (!$checkUserCallResult->isSuccess()) {
        $checkUserCallResult->throwException();
    }
